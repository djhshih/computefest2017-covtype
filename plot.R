node_barplot_rot <- function (ctreeobj, col = "black", fill = NULL, beside = NULL, 
    ...
    rval <- function(node) {
        ...
        if (beside) {
            ...
            grid.text(ylevels, x = xcenter, y = unit(-1, "lines"), 
                ...
                check.overlap = TRUE, rot=45)
            grid.yaxis(name = "frequency")
        }
        else {
...
class(node_barplot_rot) <- "grapcon_generator";
